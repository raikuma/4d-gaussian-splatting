	...
	const int iteration)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		tan_fovx,
		tan_fovy,
		active_tile_mask.numel() > 0 ? active_tile_mask.contiguous().data_ptr<int>() : nullptr,
		active_tile_ids.numel() > 0 ? active_tile_ids.contiguous().data_ptr<int>() : nullptr,
		static_cast<int>(active_tile_ids.numel()),
		prefiltered,
